...
public:
    static NodeDef* CreateConstNode(GraphDef* graph,
                                    const string& name,
                                    const std::vector<int32>& values,
                                    const string& device);

    static NodeDef* InsertTranspose(GraphDef* graph,
    ...
    static void RedirectEdges(GraphDef* graph,
                              const string& old_node_name,
    ...
    static NodeDef* InsertCast(GraphDef* graph, const string& name, const string& input_name,
                               DataType src_dtype, DataType dst_dtype, const string& device);
};
