																																			                            ...
																																			                            NodeDef* node = optimized_graph->mutable_node(i);
																																						                            ...
																																						                            if (!MusaGraphUtils::IsMusaNCHWSupported(*node)) {
																																									                            ...
																																									                            auto* attr = node->mutable_attr();
																																												                            bool is_already_nchw = (attr->count("data_format") && (*attr)["data_format"].s() == "NCHW");
																																															                            if (is_already_nchw) continue;

																																																		                            bool has_nchw_upstream = false;
																																																					                            if (node->input_size() > 0) {
																																																									                                if (node->input(0).find("/post_transpose_nhwc") != std::string::npos) {
																																																								                            ...
																																																								                            bool should_transform = false;
																																																											                            if (MusaGraphUtils::kLayoutSensitiveOps(*node)) {
																																																														                            ...
																																																														                            else if (MusaGraphUtils::kLayoutAgnosticOps(*node) && has_nchw_upstream) {
																																																																		                                should_transform = true;
																																																																						                        }
																																																																									                            ...
																																																																									                            DataType dtype = (*attr)["T"].type();
																																																																												                                std::string device = node->device();
																																																																																                            ...
																																																																																                            if (has_nchw_upstream) {
																																																																																				                                    std::string real_src = node->input(0).substr(0, node->input(0).find("/post_transpose_nhwc"));
																																																																																								                                    node->set_input(0, real_src);
																																																																																												                                }
																																																																																			                                else {
																																																																																								                                std::string pre_name = op_name + "/pre_transpose_nchw";
																																																																																												                                MusaGraphUtils::InsertTranspose(optimized_graph, pre_name, node->input(0),
