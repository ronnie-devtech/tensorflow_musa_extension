...
mStatus MusaMemcpyH2D(void* d, const void* h, size_t size) {
    ...
    if (err != musaSuccess) {
        // fprintf(stderr, ">>> [MUSA] H2D Sync Failed: %s, size: %zu, src: %p, dst: %p\n",
...
mStatus MusaMemcpyAsyncD2H(void* h, const void* d, size_t size, musaStream_t s) {
    if (size == 0) {
        return mStatus::SUCCESS;
    }
    ...
    if (err != musaSuccess) {
        // fprintf(stderr, ">>> [MUSA] D2H Async Failed: %s, size: %zu, stream: %p\n",
...
mStatus MusaMemcpyAsyncH2D(void* d, const void* h, size_t size, musaStream_t s) {
    if (size == 0) {
        return mStatus::SUCCESS;
    }
    ...
    if (err != musaSuccess) {
        // fprintf(stderr, ">>> [MUSA] H2D Async Failed: %s, size: %zu, stream: %p\n",
        //         musaGetErrorString(err), size, s);
        // return static_cast<mStatus>(1);
    }
    return mStatus::SUCCESS;
...
mStatus MusaMemcpyAsyncD2D(void* d1, const void* d2, size_t size, musaStream_t s) {
    if (size == 0) {
        return mStatus::SUCCESS;
    }
    ...
    if (err != musaSuccess) {
        // fprintf(stderr, ">>> [MUSA] D2D Async Failed: %s, size: %zu, stream: %p\n",
        //         musaGetErrorString(err), size, s);
        // return static_cast<mStatus>(1);
    }
    return mStatus::SUCCESS;
