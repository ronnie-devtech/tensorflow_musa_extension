...
class MusaReshapeOp : public MusaOpKernel {
 public:
  explicit MusaReshapeOp(OpKernelConstruction* context) : MusaOpKernel(context) {
  }
...
//    fprintf(stderr, ">>> [MUSA_TRACE_AUTO] %s\n", name().c_str());
      ...
      for (int i = 0; i < vec.size(); ++i) {
        ...
        if (size == -1) {
          ...
          OP_REQUIRES(ctx, size >= 0,
                      errors::InvalidArgument("Dimension size must be non-negative, got ", size));
          shape.AddDim(size);
          product *= size;
        }
      }
    ...
    if (unknown_index != -1) {
      ...
      OP_REQUIRES(ctx, product > 0,
                  errors::InvalidArgument("Cannot infer -1 dimension with zero product"));
      OP_REQUIRES(ctx, input_num_elements % product == 0,
                  errors::InvalidArgument("Input has ", input_num_elements,
