    ...
    fprintf(stderr, ">>> [MUSA_TRACE_AUTO] %s\n", name().c_str());
                ...
                if (std::is_same<T, float>::value) {
                    status = mublasSgemmStridedBatched(
                        ...
                        m, n, k,
                        &alpha,
                        (const float*)d_A, k, strideA, // lda = k
                        (const float*)d_B, k, strideB, // ldb = k
                        &beta,
                        (float*)d_C, m, strideC,       // ldc = m
                        batch_size
                    );
                } else {
                    OP_REQUIRES(ctx, false, errors::Unimplemented("Only float32 is supported in this demo."));
                    return;
                }

                OP_REQUIRES(ctx, status == MUBLAS_STATUS_SUCCESS,
                            errors::Internal("muBLAS SgemmStridedBatched failed. Status: ", status));

            }
        };

        REGISTER_KERNEL_BUILDER(Name("MusaInteract")
                                    .Device("MUSA")
