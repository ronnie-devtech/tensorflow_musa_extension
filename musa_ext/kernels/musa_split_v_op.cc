 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, split_dim_tensor.NumElements() == 1,
                errors::InvalidArgument("split_dim_tensor must have exactly one element."));
    ...
    OP_REQUIRES(context, split_tensor.dims() == 1 && split_tensor.NumElements() == num_split,
                errors::InvalidArgument("size of the split_tensor must be 1-D and match num_outputs"));
    ...
    Tlen input_size_split_dim = static_cast<Tlen>(input_shape.dim_size(split_dim));
    ...
    for(auto s : split_sizes_vec) {
        OP_REQUIRES(context, s >= 0, errors::InvalidArgument("Split size must be >= 0"));
        total_len += s;
    }
    OP_REQUIRES(context, total_len == input_size_split_dim,
                errors::InvalidArgument("Sum of split sizes must match input dim size"));
    ...
    for (int i = 0; i < num_split; ++i) {
      ...
      OP_REQUIRES_OK(context, context->allocate_output(i, out_shape_base, &output));
