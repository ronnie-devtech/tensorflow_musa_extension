...
class MusaLayerNormOp : public MusaOpKernel {
 public:
  explicit MusaLayerNormOp(OpKernelConstruction* ctx)
      : MusaOpKernel(ctx) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(ctx, gamma.NumElements() == last_dim,
                errors::InvalidArgument("Gamma size mismatch: expected ", last_dim, 
                                        ", got ", gamma.NumElements()));
    OP_REQUIRES(ctx, beta.NumElements() == last_dim,
    ...
    auto alloc_func = [tf_allocator](size_t size) -> std::unique_ptr<void, std::function<void(void*)>> {
        ...
        return std::unique_ptr<void, std::function<void(void*)>>(ptr, [tf_allocator](void* p) {
